...
extension PackageManager.SymbolGraphOptions {
    ...
    mutating func override(with arguments: Arguments) {
        for argument in arguments {
            switch argument {
            case "--omit-extension-block-symbols":
#if swift(>=5.8)
                self.emitExtensionBlocks = false
#else
                print("warning: detected '--omit-extension-block-symbols' option, which is incompatible with your swift version (required: 5.8)")
#endif
            default:
